...
connection.dbConnection.once('open', function () {
    ...
    connection.UserAuth.countDocuments({'username':'admin'}, function(err, count){
        if(err) throw err;
        ...
        if(count==0){
            ...
            new connection.User({firstName: "Anupam",lastName: "Chowdhury"})
                          .save(function (err, results) {
                            if(err) throw err;
                          
                            new connection.UserAuth({username:'admin',password:password,user:results._id})
                                          .save(function(err, results){
                                               if(err) throw err;
